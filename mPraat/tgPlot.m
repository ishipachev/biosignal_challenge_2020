...
%         title('interval tier')
        StemHandle = stem(tg.tier{I}.T1, 2*0.8*ones(size(tg.tier{I}.T1)), 'MarkerSize', 1, 'LineWidth', 2);
        hold on
        stem(tg.tier{I}.T2, 2*0.5*ones(size(tg.tier{I}.T2)), 'MarkerSize', 1, 'LineWidth', 2);
        for K = 1: length(tg.tier{I}.T1)
%             plot([tg.tier{I}.T1(K) tg.tier{I}.T2(K)], 2*[0.5 0.5]);
              plot([tg.tier{I}.T1(K) tg.tier{I}.T2(K)], 2*[0.8 0.5], 'LineWidth', 2);
        end
        hold off
